...
namespace MoveUnitTest {
    ...
    TEST(Move, toStringFigure) {
        auto move = Move(Position(8, 2), Position(6, 3), new Knight(), false);
        ASSERT_EQ("Nc6", move.toString());
    }

    TEST(Move, toStringPawn) {
        auto move = Move(Position(2, 4), Position(4, 4), new Pawn(), false);
        ASSERT_EQ("d4", move.toString());
    }

    TEST(Move, toStringFigureCapture) {
        auto move = Move(Position(3, 2), Position(6, 5), new Queen(), true);
        ASSERT_EQ("Qxe6", move.toString());
    }

    TEST(Move, toStringPawnCapture) {
        auto move = Move(Position(7, 2), Position(8, 1), new Pawn(), true);
